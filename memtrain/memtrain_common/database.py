...
class Database:
    ...
    def __init__(self):
        ...
        self.conn.execute(
            """CREATE TABLE cues_to_responses
                          (cue_id INTEGER,
                          response_id INTEGER,
                          placement INTEGER,
        ...
        self.conn.execute(
            """CREATE TABLE responses_to_synonyms
                          (response_id INTEGER,
                          synonym_id INTEGER,
        ...
        self.conn.execute(
            """CREATE TABLE responses_to_hints
                          (response_id INTEGER,
                          hint_id INTEGER,
        ...
        self.conn.execute(
            """CREATE TABLE responses_to_tags
                          (response_id INTEGER,
                          tag_id INTEGER,
        ...
        self.conn.execute(
            """CREATE TABLE responses_to_mtags
                          (response_id INTEGER,
                          mtag_id INTEGER,
        ...
        for data_row in data_list:
            ...
            for placement, index in enumerate(indices["response"]):
                ...
                if response:
                    ...
                    self.conn.execute(
                        """INSERT INTO cues_to_responses(cue_id, response_id, placement)
                                VALUES (?,?,?)""",
                        (cue_id, response_id, placement),
                    )
                    ...
                    for index in indices["synonym"][placement]:
                        ...
                        if synonym:
                            ...
                            if pair not in responses_to_synonyms_list:
                                self.conn.execute(
                                    """INSERT INTO responses_to_synonyms(response_id, synonym_id)
                                                VALUES (?,?)""",
                                    pair,
                                )
                    ...
                    for index in indices["hint"][placement]:
                        ...
                        if hint:
                            ...
                            if pair not in responses_to_hints_list:
                                self.conn.execute(
                                    """INSERT INTO responses_to_hints(response_id, hint_id)
                                            VALUES (?,?)""",
                                    pair,
                                )
                    ...
                    for tag_index in indices["tag"]:
                        ...
                        if tag:
                            ...
                            if pair not in responses_to_tag_list:
                                self.conn.execute(
                                    """INSERT INTO responses_to_tags(response_id, tag_id)
                                            VALUES (?,?)""",
                                    pair,
                                )
                    ...
                    for mtag_index in indices["mtag"]:
                        ...
                        if mtag:
                            ...
                            if pair not in responses_to_mtag_list:
                                self.conn.execute(
                                    """INSERT INTO responses_to_mtags(response_id, mtag_id)
                                            VALUES (?,?)""",
                                    pair,
                                )
    ...
    def get_all_response_ids_by_tag(self, tag):
        ...
        tag_id = rows[0][0]
        self.cur.execute(
            """SELECT response_id FROM responses_to_tags
                            WHERE tag_id = (?)""",
            (str(tag_id),),
        )
        rows = self.cur.fetchall()
