...
class MemtrainGUI:
    ...
    def configure_styles(self):
        ...
        self.style.configure(
            "SectionTitle.TLabel", background=background, font=("TkDefaultFont", 13, "bold")
        )
        self.style.configure(
            "CardSectionTitle.TLabel", background=surface, font=("TkDefaultFont", 13, "bold")
        )
        self.style.configure("Muted.TLabel", background=background, foreground=muted)
        self.style.configure(
            "CardTitle.TLabel", background=surface, font=("TkDefaultFont", 14, "bold")
        )
        self.style.configure("CardBody.TLabel", background=surface, font=("TkDefaultFont", 12))
        self.style.configure(
            "Meta.TLabel", background=surface, foreground=muted, font=("TkDefaultFont", 11)
        )
    ...
    def build_home_ui(self):
        ...
        tk_ttk.Label(header, text="memtrain", style="SectionTitle.TLabel").grid(
        ...
        tk_ttk.Button(
            self.settings_card, text="Open Settings", command=self.configure_settings
        ...
        self.select_csv_button = tk_ttk.Button(
            self.file_card, text="Choose CSV Study Set", command=self.select_csv
        )
        self.select_csv_button.grid(row=0, column=0, sticky="w")

        self.step_1_filename_label = tk_ttk.Label(
            self.file_card, text="No file selected.", style="Muted.TLabel", wraplength=640
        )
        self.step_1_filename_label.grid(row=1, column=0, sticky="ew", pady=(12, 0))

        self.start_card = tk_ttk.LabelFrame(
            self.root_frame, text="Step 2: Start Session", padding=16
        )
    ...
    def configure_settings(self):
        ...
        tk_ttk.Label(frame, text="Questions", style="SectionTitle.TLabel").grid(
        ...
        tk_ttk.Label(frame, text="Number of questions").grid(
        ...
        tk_ttk.Button(buttons, text="Clear", command=self.clear_settings).grid(
            row=0, column=0, padx=(0, 8)
        )
        tk_ttk.Button(
            buttons, text="Save", command=self.commit_settings, style="Primary.TButton"
        ).grid(row=0, column=1)
    ...
    def configure_start_buttons(self):
        ...
        self.level_1_button.configure(
            state=tk.NORMAL if self.settings.settings["level1"] else tk.DISABLED
        )
        self.level_2_button.configure(
            state=tk.NORMAL if self.settings.settings["level2"] else tk.DISABLED
        )
        self.level_3_button.configure(
            state=tk.NORMAL if self.settings.settings["level3"] else tk.DISABLED
        )

    def start_level(self, level):
        if not self.filename:
            tk_messagebox.showinfo(
                "Select a study set", "Choose a CSV file before starting.", parent=self.root
            )
            return
    ...
    def build_training_window(self):
        ...
        self.cue_label = tk_ttk.Label(
            self.prompt_card, style="CardBody.TLabel", justify=tk.LEFT, wraplength=760
        )
        ...
        self.feedback_label.configure(
            text="After your first response, result feedback will appear here."
        )

        self.other_answers_label = tk_ttk.Label(
            self.prompt_card, style="Meta.TLabel", justify=tk.LEFT, wraplength=760
        )
        ...
        self.hint_title_label = tk_ttk.Label(
            self.hint_frame, text="Hint", style="CardSectionTitle.TLabel"
        )
        self.hint_title_label.grid(row=0, column=0, sticky="nw")
        self.hint_text_label = tk_ttk.Label(
            self.hint_frame, style="CardBody.TLabel", justify=tk.LEFT, wraplength=680
        )
        ...
        self.button = tk_ttk.Button(
            self.response_frame, text="Submit", command=self.submit, style="Primary.TButton"
        )
    ...
    def render_question(self):
        ...
        if self.settings.level == "1":
            ...
            for row_index, (letter, choice) in enumerate(self.question.mchoices.items()):
                ...
                label = tk_ttk.Label(
                    self.mchoices_frame, text=choice, style="CardBody.TLabel", wraplength=660
                )
    ...
    def submit(self, event=None, mchoice_letter=None):
        ...
        if self.mtstatistics.is_input_valid:
            ...
            self.engine.record_result(
                self.current_item, self.mtstatistics.is_input_correct, elapsed_time
            )
